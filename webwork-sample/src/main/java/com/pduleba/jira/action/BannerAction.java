 ...
 * com.atlassian.jira.web.action.admin.EditAnnouncementBanner but could just as
 * well extend that class if so desired.
 *
 * To enable logging for this class, add this to log4j.properties:
 *
 * log4j.logger.com.atlassian.jira.web.action.custom.EditAnnouncementBanner =
 * DEBUG, console, filelog
 * log4j.additivity.com.atlassian.jira.web.action..custom.EditAnnouncementBanner
 * = false
 */
public class BannerAction extends JiraWebActionSupport {
	...
	public BannerAction(ApplicationProperties applicationProperties) {
	...
	public String doDefault() throws Exception {
		...
		announcement = (preview == null) ? applicationProperties.getDefaultBackedText(APKeys.JIRA_ALERT_HEADER)
				: preview;
	...
	public Collection getVisibilityModes() {
		return EasyList.build(
				new TextOption(PUBLIC_BANNER,
						getText("admin.menu.optionsandsettings.announcement.banner.visibility.public")),
				new TextOption(PRIVATE_BANNER,
						getText("admin.menu.optionsandsettings.announcement.banner.visibility.private")));
	}
